...
function createSearchResultRow(result) {
    ...
    if (result.tags) {
        ...
        if (typeof result.tags === 'string') {
            try {
                ...
                // 如果不是JSON，按逗号分割
                if (result.tags !== '无') {
                    tags.push(...result.tags.split(',').map(t => t.trim()).filter(Boolean));
                }
            }
    ...
    // 情感分析标签
    const sentimentHtml = `
        <div class="sentiment-tag-container">
    ...
    articleDetail.innerHTML = `
        <div class="article-content">
            ...
            <div class="article-meta">
                ...
                <span class="sentiment-level">情感等级: ${result.sentiment_level || '未知'}</span>
            </div>
