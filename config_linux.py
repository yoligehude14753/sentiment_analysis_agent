...
class Config:
    ...
    # 服务器配置 - Linux环境适配
    HOST = os.getenv("HOST", "127.0.0.1")  # 生产环境使用127.0.0.1，通过Nginx代理
    PORT = int(os.getenv("PORT", 8000))
    DEBUG = os.getenv("DEBUG", "False").lower() == "true"  # 生产环境默认关闭调试
    
    # 数据文件路径 - Linux路径适配
    DATA_FILE_PATH = os.getenv("DATA_FILE_PATH", "/var/www/sentiment-analysis/data/sentiment_data.csv")
    
    # 分析配置
    MAX_CONTENT_LENGTH = int(os.getenv("MAX_CONTENT_LENGTH", "2000"))  # 最大内容长度
    BATCH_SIZE = int(os.getenv("BATCH_SIZE", "100"))  # 批处理大小 

    # 日志配置
    LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO")
    LOG_FILE = os.getenv("LOG_FILE", "/var/www/sentiment-analysis/logs/app.log")
